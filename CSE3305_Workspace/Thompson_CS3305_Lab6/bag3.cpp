...
namespace main_savitch_5
{
    ...
    {
        bag new_bag(subtrahend);
        while(new_bag.head_ptr != NULL)
        {
            value_type value = new_bag.head_ptr->data();
            node* node = list_search(head_ptr, value);
            if(node != NULL && node->data() == value)
                erase_one(value);
            list_head_remove(new_bag.head_ptr);
        }
    }
